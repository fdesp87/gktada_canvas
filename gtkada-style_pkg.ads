...
package Gtkada.Style_Pkg is
   ...
   type Side_Anchor_Style is (Not_Defined, Top, Right, Bottom, Left);
   --  Used internally

   type Route_Style is (Orthogonal, Straight, Line, Spline, Rounded);
   --  This defines how a link is routed between its two ends.
   --  Rounded is similar to orthogonal (links restricted to horizontal and
   ...
   function "+" (P, Q : Point) return Point;
   function "*" (K : Glib.Gdouble; P : Point) return Point;
   function "*" (P : Point; K : Glib.Gdouble) return Point;
   function "/" (P : Point; K : Glib.Gdouble) return Point;
   function "-" (P, Q : Point) return Point;
   function Arctan (P1, P2 : Point) return Glib.Gdouble; --  radians
